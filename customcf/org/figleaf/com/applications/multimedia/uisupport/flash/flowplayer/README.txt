...
- added new "combine" variable for clips. If true (default) the clip will be combined to the previous clip
  so that it will be played as one stream when RTMP is used
- New backgroundColor variable that controls the color of the player's background. Specifying -1 causes
the background not to be drawn.
- pause/resume button stays enabled when controlEnabled: false is used. Now the video can be resumed
  if the user comes back from the advertised (linked) web site
Fixes:
...
2.2.2
	- flow.embed has a new fallback parameter you can can use play high-quality
	videos in h.264 format so that the player will automatically offer FLV
	videos for users that don't have Flash 9.0.115 or greater that is needed to support h.264
	- new version of flashembed included with fixes for Mac Safari and Internet Explorer

...
<cffunction name="init" returntype="Validator" access="public" output="false" 
	...
	<cfset variables.config = application.BeanFactory.getBean("configBean") />
	<cfset variables.logger = application.BeanFactory.getBean("logger") />
	<cfset variables.FileSystem = application.BeanFactory.getBean("FileSystem") />
	
	<!--- list of forbidden strings --->
	<cfset variables.forbiddenStrings = "">
	
	<!--- read the xml --->
	<cfset thePath = variables.config.getFigLeafFilePath() & "/com/applications/validator/" />
